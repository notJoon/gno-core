...
func TestFormatBenchmarkResult(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			benchmem: true,
			want:     name + "\t10\t420 cycles/op\t500 gas/op\t7 bytes/op\t100 B/op\t5 allocs/op",
		},
		{
			desc:     "zero_n_uses_one_as_divisor",
			rep:      benchmarkReport{N: 0, Cycles: 4200, Gas: 5000, AllocBytes: 1000, Allocs: 50},
			benchmem: true,
			want:     name + "\t0\t4200 cycles/op\t5000 gas/op\t1000 B/op\t50 allocs/op",
		},
	}
